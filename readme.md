...
在 Spring Cloud 的早期版本中，服务容错和保护主要依赖于 Hystrix 实现，而 Hystrix 是 Netflix 开源的一款断路器组件，能够有效应对服务雪崩效应。但随着其停止维护，
Spring Cloud 官方逐步将其淘汰，并推出了新的服务保护抽象框架 Spring Cloud Circuit Breaker。该框架提供统一的断路器接口，底层可灵活选择不同的实现方案，
如 Spring Retry 和 Resilience4J 等。不过，在国内的微服务开发实践中，更常见的选择是 Spring Cloud Alibaba 提供的 Sentinel 组件。

### 4.1 线程隔离

在微服务架构中，线程隔离是服务容错（如熔断、降级）中的一种关键手段，无论是 Hystix 还是 Sentinel 都支持线程隔离，它们的目的都是防止某个服务调用发生阻塞或过载，
导致拖垮整个系统，不过它们的实现方式不同。线程隔离有两种方式实现：

- 线程池隔离：给每个服务分配一个独立的线程池，服务调用在线程池中异步执行，与主线程解耦。但资源占用较高（每个服务一个线程池）。
- 信号量隔离：不创建线程池，而是使用计数器模式，记录业务使用的线程数量，达到信号量上限时，新请求会被拒绝或降级处理。
...
在微服务系统中，无论是实现熔断还是限流，都需要对请求进行实时统计与分析，尤其是对异常请求数量、慢调用比例、QPS（单位时间内请求总数）等指标的计量。
这些都依赖于高效且准确的计数算法，而sentinel中采用的计数器算法就是滑动窗口计数算法。

#### 4.2.1 固定窗口计数

固定窗口计数是一种按照固定时间间隔统计请求次数的限流算法，将时间轴划分为一个个大小相等的窗口（如每 1 秒一个窗口），每个窗口内记录请求数量，当请求到达时，
如果当前窗口内的请求数还没有超过阈值，就放行；否则拒绝或降级。例如：
...
每个窗口内维护一个请求计数器：
窗口0（0~1s）：count = 100 -> 满了
窗口1（1~2s）：count = 50  -> 还能进
窗口2（2~3s）：count = 0   -> 新窗口刚开始
```
...
但是该窗口计数方式存在一定的问题，例如在第 5~6s，请求数量都为 100，没有超过阈值，全部放行。但是，如果第 5s 的 100 次请求都是在 4.5~5s 之间进来；
第 6s 的请求是在 5~5.5s 之间进来。那么从第 4.5~5s 之间就有 200 次请求，也就是说每秒的 QPS 达到了 200，远超阈值。这两个时间点非常接近，
但总共已经收到 200 个请求在不到 1s 的时间内，这就会造成瞬时过载。
...
请求流：
- 第 1 秒的最后 1 毫秒：来了 100 个请求
- 第 2 秒的前 1 毫秒：又来了 100 个请求

-> 实际上在不到 2ms 内来了 200 个请求，但算法认为它们属于两个不同的窗口，就没有限流
```

****
#### 4.2.2 滑动窗口计数

固定时间窗口算法与滑动时间窗口算法的本质区别就体现在对 "窗口的划分方式" 与 "计数粒度" 的不同上：

固定窗口：

- 窗口数量：理论上是无限多个，每个窗口独立存在，位置固定
- 窗口划分方式：以时间轴为基准，将时间划分为多个等长、连续、不重叠的固定区间
- 窗口编号：与时间绝对值绑定（例如每秒一个窗口编号：1s、2s、3s...）
- 统计范围：每次只统计当前时间所处的整个固定窗口内的计数

滑动窗口：

- 窗口数量：只有一个活动窗口，但由多个小格子组成
- 窗口划分方式：定义一个总跨度（如 1s），划分为多个小格子（如 10 个，每个 100ms）
- 窗口位置：随着时间推进，窗口会滑动，并不断淘汰过期格子，引入新格子
- 统计范围：统计当前时刻往前推一个完整窗口跨度内的所有小格子之和

```text
时间轴（假设窗口跨度为 1 秒）：
当前时间：2.5s
统计范围：1.5s ~ 2.5s 之间的请求数（由多个 bucket 组成）
...
假设限流阈值为 3，每个请求携带 currentTime 值

- 如果在第 1300ms 时接收到一个请求，那么其所在时区就是 1000~1500
- 按照规则，currentTime-Interval 值为 1300 - 1000 = 300ms，300ms 对应的时区的后面一个时区是 500~1000ms，因此窗口范围包含两个时区：500~1000、1000~1500ms
- 统计窗口内的请求总数，发现是 3，未达到上限
- 也就是说本应该判断 1000~2000ms 的方格被拆分为判断 500~1000、1000~1500ms
...
currentTime = 1300ms
-> 所在格子：      [1000 ~ 1500)
滑动窗口 = [300ms ~ 1300ms] 实际覆盖格子为：
-> Bucket1:       [500 ~ 1000)
-> Bucket2:       [1000 ~ 1500)
```

虽然滑动窗口比固定窗口更灵活，但如果一时间请求过多还是会造成无法正确统计，所以划分的时区越多，这种统计就越准确。

****
#### 4.2.3 漏桶算法

> 漏桶算法（Leaky Bucket）是一种流量整形和限流的常用算法，常用于网络流量控制和分布式系统中的服务保护机制，它通常用来控制请求的速率，防止突发流量对系统造成冲击。

漏桶算法可以类比为一个装水的漏桶（桶有固定容量），请求相当于水滴，被放入漏桶中；漏桶以固定速率漏水，代表系统以稳定速率处理请求；如果水太快流入，桶满了就溢出，
也就是请求被丢弃（拒绝）；因为处理速率固定，即使短时间请求量很大，系统仍以平稳速度处理，不会因为突发流量而过载。
...
限流的另一种常见算法是令牌桶算法，Sentinel 中的热点参数限流正是基于令牌桶算法实现的。可以将令牌桶算法类比为一个可以产生 "令牌" 的水桶，系统会以固定速率（如每秒 5 个）往桶里放令牌；
不过桶有容量限制，最多只能存 b 个令牌，每个请求要处理，就必须先获取一个令牌，如果桶里有令牌，则立即处理请求并消耗一个令牌；如果没有令牌，说明请求超过限流速率，请求被丢弃或排队。

基于令牌桶算法，每秒产生的令牌数量基本就是 QPS 上限。当然也有例外情况，例如：

- 某一秒令牌桶中产生了很多令牌，达到令牌桶上限 N，缓存在令牌桶中，但是这一秒没有请求进入
- 下一秒的前半秒涌入了超过 2N 个请求，之前缓存的令牌桶的令牌耗尽，同时这一秒又生成了 N 个令牌，于是总共放行了 2N 个请求，这就超出了设定的 QPS 阈值
